...
auth_routes_code = '''from fastapi import APIRouter, Request, Form, HTTPException, status, Depends
...
def handle_login(
    ...
    with get_db() as db:
        ...
        if user["status"] == "pending_approval":
            return templates.TemplateResponse(
                "auth/login.html",
                {"request": request, "next": next, "error": "Your registration is currently pending Administrator approval. You will receive access once approved."},
                status_code=403
            )
        elif user["status"] != "active":
            return templates.TemplateResponse(
                "auth/login.html",
                {"request": request, "next": next, "error": "Your account is inactive. Please contact the MoES Capacity Building Cell."},
                status_code=403
            )
...
def handle_register(
    ...
    with get_db() as db:
        ...
        if cursor.fetchone():
            return templates.TemplateResponse(
                "auth/register.html",
                {"request": request, "error": "An account with this email address already exists. Please login instead.", "success": None},
                status_code=400
            )
...
with open("app/routes/auth_routes.py", "w", encoding="utf-8") as f:
    f.write(auth_routes_code)

print("app/routes/auth_routes.py created successfully")
